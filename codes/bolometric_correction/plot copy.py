...
sed = data['logall']
id = (lamb - con.c.value/(2.*1000.*con.e.value/con.h.value)*1e10)<0
scale =  np.log10( f2kev*(2.*1000.*con.e.value/con.h.value) ) - sed[id][0]
ax.plot(lamb, sed + scale,'-',c='royalblue',label=r'$\rm Xray$ $\rm Hopkins+$ $\rm 2007$')
...
id = (lamb - con.c.value/(2.*1000.*con.e.value/con.h.value)*1e10)<0
scale =  np.log10( f2kev*(2.*1000.*con.e.value/con.h.value)) - sed[id][0]
ax.plot(lamb, sed + scale,'--',dashes=(15,9),c='crimson',label=r'$\rm Xray$ $\rm Hopkins+$ $\rm 2007$')
