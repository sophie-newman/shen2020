...
f50kev=1
data=np.genfromtxt("Xspec_refl.dat",names=["E","f"],)
lamb=con.c.value/(data['E']*1000*con.e.value/con.h.value)*1e10
sed = np.log10(data['f']*data['E'])
id = (lamb - con.c.value/(50.*1000.*con.e.value/con.h.value)*1e10)<0
scale =  np.log10( f50kev*(50.*1000.*con.e.value/con.h.value)) - sed[id][0]
ax.plot(lamb, sed + scale,'-',c='cyan',label=r'$\rm repro$',alpha=0.4)

f50kev=1
data=np.genfromtxt("Xspec_Nie16.dat",names=["E","Ef"],)
lamb=con.c.value/(data['E']*1000*con.e.value/con.h.value)*1e10
sed = np.log10(data['Ef'])
id = (lamb - con.c.value/(50.*1000.*con.e.value/con.h.value)*1e10)<0
scale =  np.log10( f50kev*(50.*1000.*con.e.value/con.h.value)) - sed[id][0]
ax.plot(lamb, sed + scale,'-',c='crimson',label=r'$\rm original$',alpha=0.4)
