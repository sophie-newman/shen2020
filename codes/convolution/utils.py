...
#def mAB_obs(log_L_band, nu_eff, redshift):
#	M_AB = -2.5*log_L_band + 2.5*log10(nu_eff)-32.38265724887536
...
def return_ratio_to_b_band(nu):
	...
	if ((log_nu_obs>=log_nu[0]) and (log_nu_obs<=log_nu[224])): 
...
def return_ratio_to_hard_xray(nu):
	...
	21.46, 21.48])
	log_nuLnu[275] = np.array([-2.1132, -2.1092, -2.1052, -2.1012, -2.0972, -2.0932, -2.0892, -2.0852, -2.0812, -2.0772, -2.0732, 
	...
	if (log_nu_obs > log_nu[273]): nuLnu_obs = -2.0 - np.power(10.0,log_nu_obs-20.1204)*(0.43429448)
	if ((log_nu_obs>=log_nu[0]) and (log_nu_obs<=log_nu[273])):
		n0 = int((log_nu_obs-log_nu[0])/0.02)
		nuLnu_obs = log_nuLnu[n0] + (log_nuLnu[n0+1]-log_nuLnu[n0]) * ((log_nu_obs-log_nu[n0])/(log_nu[n0+1]-log_nu[n0]))
