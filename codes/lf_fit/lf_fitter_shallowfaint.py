...
parameters_init = np.array([ 0.369, -0.575, 2.475, -1.00, 1.08, -3.60, -0.353, 12.97, -0.58, 0.47])
parameters_vary = np.array([ 0.15 , 0.25  , 0.7  , 0.5  , 0.5 , 1.6  , 0.2   , 3.   , 0.25 , 0.25])
...
def get_fit_data(alldata,parameters,zmin,zmax,dset_name,dset_id):
	...
	for iz in range(len(z_lis)):
		...
		if (len(L_data) > 0):
			...
			bolLF_model = LF_at_z(L_bol_grid,parameters,redshift,"ShallowFaint")
...
def lnprior(pars):
	...
	zdummy = 4.
	xsi = 1.+ zdummy
	gamma1=powerlaw_gamma1(zdummy,(P[0],zref,P[1]))
	gamma2=doublepower(zdummy,[P[2],zref,P[3],P[4]])
	Phis  =P[5]*T0(xsi)+P[6]*T1(xsi)
	Lbreak=doublepower(zdummy,[P[7],zref,P[8],P[9]])
...
if os.path.isfile("output_shallowfaint/chain_end.dat")==True:
	pos = np.genfromtxt("output_shallowfaint/chain_end.dat")[:,2:]

if os.path.isfile("output_shallowfaint/chain.dat")==False:
	f = open("output_shallowfaint/chain.dat", "w")
	f.close()
