...
def load_fiore12_lf_data(z): # L_HX, PHI_HX, DPHI_HX, z
	...
	WHICH_BLOCK=4
	if (z > 3.)  and (z < 4.): WHICH_BLOCK = 1
	if (z > 4.)  and (z < 5.): WHICH_BLOCK = 2
	if (z > 5.8) and (z < 7.5):WHICH_BLOCK = 3

	if (WHICH_BLOCK == 4): return False
	elif (WHICH_BLOCK == 1):
		...
		L_HX_high=np.array([43.00])
		L_HX_low =np.array([44.00])
		P_HX = np.log10(np.array([2.1e-5]))
		D_HX_high = np.array([0.9])*1e-5
		D_HX_low  = np.array([0.7])*1e-5
	elif (WHICH_BLOCK == 3):
		L_HX_high=np.array([43.50])
		L_HX_low =np.array([44.50])
