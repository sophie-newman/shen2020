...
L_B = bolometric_correction(L_bol_grid,-1)
nu_c = c_double(-1)
input_c= np.power(10.,LF(L_bol_grid,parameters_g)).ctypes.data_as(ctypes.POINTER(ctypes.c_double))
res = convolve_c(input_c,nu_c)
res = [i for i in res.contents]
PHI_B = np.array(res,dtype=np.float64)
x = (M_sun_Bband_AB -2.5*L_B) + 0.706
y = np.log10(PHI_B) - np.log10(2.5)
ax.plot(x,y,'--',dashes=(25,15),c='chocolate',label=r'$\rm new$ $\rm global$ $\rm fit$')
...
ax.minorticks_on()
#plt.savefig("../figs/test"+str(redshift)+".pdf",fmt='pdf')
plt.show()
