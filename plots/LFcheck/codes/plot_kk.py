...
def get_data(zmin,zmax,parameters=parameters_init,newdata=False):
	...
	if newdata==True:
		for key in kk_dset_ids.keys():
                        get_fit_data(alldata,parameters,new_zmins[key],new_zmaxs[key],key,kk_dset_ids[key],newdata=True)
...
ax.minorticks_on()
plt.savefig("../figs/fig"+str(redmin)+"-"+str(redmax)+"_kk.pdf",fmt='pdf')
#plt.show()
